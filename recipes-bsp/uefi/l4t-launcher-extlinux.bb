...
do_sign_files() {
    ...
    if [ -n "${L4T_UBOOT_EXTLINUX_FDT_COMPAT}" ]; then
        files_to_sign="$files_to_sign ${L4T_UBOOT_EXTLINUX_FDT_COMPAT}"
    fi
...
do_install() {
    ...
    if [ -n "${L4T_UBOOT_EXTLINUX_FDT_COMPAT}" ]; then
        install -m 0644 ${B}/${L4T_UBOOT_EXTLINUX_FDT_COMPAT}* ${D}${L4T_EXTLINUX_BASEDIR}/
    fi
    if [ -n "${UBOOT_EXTLINUX_FDTOVERLAYS}" ]; then
